...
@TeleOp(name="Dancing Robot Manual1")

public class manualDR1 extends BasicOpMode_Iterative {
    ...
    public void loop() {
        ...
        boolean hArmR = gamepad1.a;
        if (hArmR) {
            arm_right.setPosition(0.5);
        }

        boolean hArmL = gamepad1.b;
        if (hArmL) {
            arm_left.setPosition(0.9);
        }

        boolean lArmR = gamepad1.x;
        if (lArmR) {
            arm_right.setPosition(0.2);
        }

        boolean lArmL = gamepad1.y;
        if (lArmL) {
